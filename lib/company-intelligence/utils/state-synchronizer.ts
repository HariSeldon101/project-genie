...
export class StateSynchronizer {
   ...
   * Note: scraper_runs table doesn't exist, saves to session's merged_data instead
   */
  async saveScraperRun(run: ScraperRun): Promise<void> {
    try {
      permanentLogger.breadcrumb('SAVE_RUN', 'Saving scraper run to session', {
        runId: run.id,
        scraperId: run.scraperId,
        status: run.status
      })

      // Save run data to session's merged_data
      // scraper_runs table doesn't exist, storing in session instead
      ...
      permanentLogger.info('Scraper run saved to session', { category: 'STATE_SYNCHRONIZER',
        runId: run.id,
        sessionId: this.sessionId
      })
    } catch (error) {
