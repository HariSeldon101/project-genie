...
export class FinancialEnricher {
  ...
  async enrich(companyName: string, domain: string): Promise<EnrichmentResult> {
    ...
    try {
      ...
      if (!ticker) {
        permanentLogger.info('FINANCIAL_ENRICHER', 'No ticker found - likely private company', { companyName})
  ...
  private async discoverTicker(companyName: string, domain: string): Promise<string | null> {
    ...
    try {
      ...
      for (const query of searchQueries) {
        permanentLogger.info('FINANCIAL_ENRICHER', 'Searching for ticker', { query})
      ...
      if (irTicker) {
        permanentLogger.info('FINANCIAL_ENRICHER', 'Ticker found on IR page', { ticker: irTicker})
        return irTicker
      }
  ...
  private async fetchMarketData(ticker: string, companyName: string): Promise<FinancialData> {
    permanentLogger.info('FINANCIAL_ENRICHER', 'Fetching market data', { ticker})
  ...
  private async extractInvestorRelations(domain: string, ticker?: string): Promise<InvestorRelations | null> {
    ...
    try {
      ...
      for (const path of irPaths) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            permanentLogger.info('FINANCIAL_ENRICHER', 'IR page found', { url})
            break
          }
        } catch {
          continue
      ...
      if (!irPageUrl || !irPageHtml) {
        permanentLogger.info('FINANCIAL_ENRICHER', 'No IR page found', { domain})
        return null
      }
