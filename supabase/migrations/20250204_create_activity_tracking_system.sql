-- Migration: Create comprehensive activity tracking system
-- Purpose: Track all user actions for audit trails and provide optimized statistics views
...
-- ACTIVITY LOG TABLE
-- ============================================
-- Stores all user actions for comprehensive audit trails
CREATE TABLE IF NOT EXISTS activity_log (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id uuid REFERENCES profiles(id) ON DELETE CASCADE,
  action text NOT NULL,
  entity_type text, -- e.g., 'scrape', 'document', 'profile', 'team'
...
-- ============================================
-- Optimized view for user statistics with all necessary joins
...
-- ============================================
-- Create historical activity entries from existing data
INSERT INTO activity_log (user_id, action, entity_type, entity_id, metadata, created_at)
SELECT
  user_id,
  'scrape_' || status,
  'company_intelligence_session',
...
WHERE NOT EXISTS (
  ...
  WHERE entity_type = 'company_intelligence_session'
  AND entity_id = company_intelligence_sessions.id::text
)
...
WHERE NOT EXISTS (
  ...
  AND entity_id = artifacts.id::text
)
ON CONFLICT DO NOTHING;
