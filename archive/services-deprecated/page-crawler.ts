...
export class PageCrawlerService {
  ...
  async crawlHomepage(): Promise<CrawlResult> {
    console.log('🕸️ [PAGE_CRAWLER] crawlHomepage() called for:', this.baseUrl)  // DEBUG
    ...
    try {
      ...
      // Fetch homepage HTML
      console.log('🕸️ [PAGE_CRAWLER] Fetching homepage HTML...')  // DEBUG
      const html = await this.fetchPage(this.baseUrl)
      console.log('🕸️ [PAGE_CRAWLER] Homepage fetch result:', {
        hasHtml: !!html,
        htmlLength: html?.length || 0
      })  // DEBUG
      if (!html) {
        console.log('🕸️ [PAGE_CRAWLER] Failed to fetch homepage, returning early')  // DEBUG
        ...
        for (const link of links) {
          ...
          if (normalizedUrl && this.isValidInternalUrl(normalizedUrl)) {
            if (!discoveredUrls.has(normalizedUrl)) {
              // PERFORMANCE FIX: Don't fetch each page, use link text or generate title
              const linkElement = $(`a[href="${link}"], a[href="${link}/"]`).first()
              const linkText = linkElement.text().trim()
              const title = linkText || generateTitleFromUrl(normalizedUrl)
   ...
   * NOTE: This should ONLY be used when we already have the page HTML
   * Never fetch pages just to get titles during discovery phase!
   */
  private async extractPageTitle(url: string, $?: cheerio.CheerioAPI): Promise<string> {
    ...
    // PERFORMANCE: Don't fetch pages during discovery - just generate title from URL
    // Fetching pages should only happen during actual scraping phase
    /*
    try {
