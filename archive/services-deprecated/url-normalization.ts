...
export class URLNormalizationService {
  ...
  // Domain is passed as parameter to methods that need it
   ...
   */
  deduplicateUrls<T extends URLEntry>(entries: T[], domain?: string): T[] {
    const startTime = Date.now()
    const seen = new Map<string, T>()
    const duplicates: string[] = []

    for (const entry of entries) {
      const normalized = this.normalizeUrl(entry.url, domain)
   ...
   */
  normalize(urls: string[], domain: string): string[] {
    return urls.map(url => this.normalizeUrl(url, domain))
  }
   ...
   */
  prioritizeUrls<T extends URLEntry>(entries: T[], domain: string): T[] {
    const startTime = Date.now()
    ...
    const prioritized = entries.sort((a, b) => {
      ...
      if (a.url === baseUrl || a.url === baseUrl + '/') return -1
      if (b.url === baseUrl || b.url === baseUrl + '/') return 1
  ...
  mergeUrlSources(
    ...
    for (const page of discoveredPages) {
      const normalized = this.normalizeUrl(page.url, domain)
   ...
   */
  isSameDomain(url: string, domain: string): boolean {
    try {
      const normalizedDomain = domain.replace(/^https?:\/\//, '').replace(/\/$/, '')
      const urlObj = new URL(url)
      return urlObj.hostname === normalizedDomain ||
             urlObj.hostname === `www.${normalizedDomain}` ||
             urlObj.hostname === normalizedDomain.replace('www.', '')
    } catch {
      return false
    }
  }
   ...
   */
  toAbsoluteUrl(url: string, domain: string): string {
   ...
   */
  generateUrlStatistics(urls: string[], domain: string): Record<string, any> {
    const normalizedDomain = domain.replace(/^https?:\/\//, '').replace(/\/$/, '')
    const baseUrl = `https://${normalizedDomain}`
    const stats = {
      total: urls.length,
      unique: new Set(urls.map(u => this.normalizeUrl(u, domain))).size,
  ...
  async batchProcessUrls(
    ...
    // Normalize all URLs
    processed = processed.map(url => this.normalizeUrl(url, domain))
    ...
    if (options.prioritize) {
      ...
      processed = this.prioritizeUrls(entries, domain).map(e => e.url)
    }
