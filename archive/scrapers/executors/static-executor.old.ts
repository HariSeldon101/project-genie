/**
 * ⚠️ DEPRECATED - ARCHIVED ON SEPTEMBER 15, 2025
 *
 * DO NOT USE THIS FILE - REPLACED BY NEW PLUGIN ARCHITECTURE
 *
 * This file has been archived as part of the scraper refactoring initiative.
 * Problems with this implementation:
 * - Monolithic design (1,219 lines - violates 500-line guideline)
 * - 60% code duplication with dynamic-executor.ts
 * - Not using shared utilities (http-fetcher, url-validator)
 * - Difficult to extend or maintain
 *
 * NEW ARCHITECTURE LOCATION:
 * /lib/company-intelligence/scrapers/plugins/static/index.ts
 *
 * NEW USAGE:
 * ```typescript
 * import { ScraperOrchestrator } from '@/lib/company-intelligence/scrapers/core/scraper-orchestrator'
 * const orchestrator = new ScraperOrchestrator()
 * const result = await orchestrator.execute(urls, options)
 * ```
 *
 * KEY IMPROVEMENTS IN NEW SYSTEM:
 * - Plugin-based architecture (150 lines max per plugin)
 * - Auto-discovery of new scrapers
 * - Shared extractors eliminate duplication
 * - 100% shared utility usage
 * - Proper repository pattern for database access
 *
 * See: /new-plugin-scraper-architecture-sept-15-2025.md for full documentation
 *
 ...
 * @deprecated Use plugin architecture instead
 */
...
export class StaticScraperExecutor extends BaseScraperExecutor {
  ...
  async scrape(urls: string[], options?: ScraperOptions): Promise<ScraperResult> {
    ...
    if (!urls || urls.length === 0) {
      ...
      permanentLogger.captureError('STATIC_EXECUTOR', new Error('No URLs provided for scraping'), {
    ...
    if (options?.urlMetadata && options.urlMetadata.size > 0) {
      ...
      permanentLogger.info('STATIC_EXECUTOR', 'URLs sorted by priority and metadata', {
