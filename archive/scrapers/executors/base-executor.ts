...
export abstract class BaseScraperExecutor implements ScraperExecutor {
  ...
  protected async startScrapingSession(urls: string[], options?: ScraperOptions): Promise<void> {
    ...
    this.correlationId = EventFactory.getCorrelationId(this.sessionId)
  ...
  protected async logPageError(url: string, error: any, duration: number): Promise<void> {
    ...
    permanentLogger.captureError(`SCRAPER_${this.id.toUpperCase()}`, error, { message: 'Page scraping failed', url,
      Message,
      stack: errorStack,
      is404,
      duration,
  ...
  protected async sendDataEvent(data: any): Promise<void> {
    ...
    const event = EventFactory.data(data, {
      source: EventSource.SCRAPER,
      phase: 'scraping',
      scraperId: this.id,
      scraperName: this.name,
  ...
  protected async sendCompleteEvent(result: ScraperResult): Promise<void> {
    ...
    const event = EventFactory.complete(result, {
      source: EventSource.SCRAPER,
      phase: 'scraping',
      scraperId: this.id,
      scraperName: this.name,
  ...
  protected createErrorResult(error: any, urls: string[]): ScraperResult {
    ...
    permanentLogger.captureError(`SCRAPER_${this.id.toUpperCase()}`, error, { message: 'Creating error result', scraperId: this.id,
      Message,
      urlCount: urls.length,
      sessionId: this.sessionId })
