...
export function PhaseControls({ domain, onPhaseComplete, hideProgressCard = false, onReset }: PhaseControlsProps) {
  ...
  const showStageToast = (stage: string, data?: any, isError: boolean = false) => {
    ...
    switch (stage) {
      ...
      case 'generation':
        persistentToast.success(`Report Generation complete! Your comprehensive research is ready. 🎉`)
        break
  ...
  const handleStageComplete = async (stage: Stage, data: any) => {
    permanentLogger.info('PHASE_CONTROLS', `Stage ${stage} completed`, { data })
    ...
    if (!sessionId && stage === 'site-analysis') {
      ...
      try {
        ...
        permanentLogger.error('Failed to create session', { category: 'PHASE_CONTROLS', { error })
      }
    }
    // Update existing session with site analysis data
      ...
      try {
        ...
        permanentLogger.error('Failed to save site analysis data', { category: 'PHASE_CONTROLS', { error })
      }
    }
  ...
  const startScraping = async (sitemapPages?: any[]) => {
    ...
    if (!sessionId) {
      permanentLogger.error('No session found for scraping', { category: 'PHASE_CONTROLS')
    ...
    if (totalPages === 0) {
      permanentLogger.error('No pages selected for scraping', { category: 'PHASE_CONTROLS')
  ...
  const startGeneration = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        persistentToast.success('Research complete! 🎉')
      } else {
        throw new Error(result.error)
      }
    } catch (error) {
                  ...
                  <div className="rounded-lg border p-4 space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant={
                        ...
                        {['react', 'nextjs', 'vue', 'angular', 'svelte'].includes(stageData['site-analysis'].siteType) 
                          ? '🎭 Playwright (Dynamic)' : '⚡ Cheerio (Static)'}
                      </Badge>
                    </div>
        ...
        {currentStage === 'data-review' && (
          <DataReviewPanel
            ...
            onProceed={(selectedData) => {
              ...
              permanentLogger.info('Data review completed', {
      category: 'PHASE_CONTROLS',
      ...
      {currentStage !== 'idle' && (
        <PersistentActionBar
          ...
          onReject={() => {
            ...
            permanentLogger.info('Process aborted by user', {
      category: 'PHASE_CONTROLS',
      currentStage,
