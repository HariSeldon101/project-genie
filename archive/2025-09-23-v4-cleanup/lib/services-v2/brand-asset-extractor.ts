...
export class BrandAssetExtractor {
  ...
  async extractBrandAssets(options: ExtractionOptions): Promise<BrandAssets> {
    ...
    permanentLogger.info('BRAND_EXTRACTOR', 'Starting brand asset extraction', {
      url,
      hasHTML: !!html,
      hasCheerio: !!$,
    ...
    try {
      ...
      if (brandAssets.colors && brandAssets.colors.length > 0) {
        ...
        permanentLogger.info('BRAND_EXTRACTOR', 'Filtered brand colors', {
          beforeCount: brandAssets.colors.length,
      ...
      permanentLogger.info('BRAND_EXTRACTOR', 'Brand extraction complete', {
  ...
  private async extractFromPage(
    ...
    if (!brandAssets.logo) {
      ...
      for (const selector of logoSelectors) {
        ...
        if (logoSrc) {
          ...
          permanentLogger.info('BRAND_EXTRACTOR', 'Found logo from selector', {
            selector,
    ...
    permanentLogger.info('BRAND_EXTRACTOR', 'Extracted colors', {
    ...
    for (const cssUrl of cssUrls) {
      try {
        ...
        if (response.ok) {
          ...
          permanentLogger.info('BRAND_EXTRACTOR', 'Extracted from CSS file', {
            url: cssUrl,
  ...
  private extractFonts($: any, brandAssets: BrandAssets) {
    ...
    permanentLogger.info('BRAND_EXTRACTOR', 'Extracted fonts', {
      count: fonts.length,
