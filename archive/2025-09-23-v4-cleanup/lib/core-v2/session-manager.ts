...
export interface SessionData {
  ...
  phase: number
  version: number
  // discovered_urls removed - now stored in merged_data.sitemap
...
export class SessionManager {
  ...
  /**
   * ⚠️⚠️⚠️ DEPRECATED - DO NOT USE - WILL CAUSE DATABASE ERRORS ⚠️⚠️⚠️
   * Deprecation Date: September 21, 2025
   *
   * 🚨 CRITICAL: This method causes UNIQUE constraint violations!
   *
   * The database has a UNIQUE constraint on (user_id, domain) which this
   * method does not properly handle, leading to:
   * - Duplicate key errors: "unique constraint 'unique_user_domain' violated"
   * - Session creation failures for returning users
   * - Race condition errors in concurrent requests
   *
   * ❌ NEVER USE:
   * ```typescript
   * await sessionManager.createSession(userId, companyName, domain)
   * ```
   *
   * ✅ ALWAYS USE:
   * ```typescript
   * await sessionManager.getOrCreateSession(userId, companyName, domain)
   * // Or even better, use repository directly:
   * await repository.getOrCreateUserSession(userId, domain)
   * ```
   *
   * WHY getOrCreateSession() IS SAFE:
   * - Checks for existing sessions first
   * - Reactivates inactive sessions
   * - Handles race conditions gracefully
   * - Respects database constraints
   *
   * @deprecated Since Sept 21, 2025 - Causes database constraint violations
   * @breaking-change Will throw duplicate key errors
   * @see CompanyIntelligenceRepository.getOrCreateUserSession() for proper implementation
   * @migration-priority CRITICAL - Prevents application functionality
   */
  async createSession(userId: string, companyName: string, domain: string): Promise<SessionData> {
    // Log deprecation warning
    this.logger.warn('SESSION_MANAGER', 'DEPRECATED METHOD CALLED: createSession()', {
      userId,
      domain,
      stack: new Error().stack,
      migration: 'Use getOrCreateSession() or repository.getOrCreateUserSession() instead'
    })

    // Redirect to the correct method to prevent breaking existing code
    // But this is still dangerous as it doesn't match the method name expectations
    return this.getOrCreateSession(userId, companyName, domain)
  }

  /**
   * Get or create a session in the database
   * CRITICAL: This method uses getOrCreateUserSession which properly handles:
   * - Existing sessions (returns them)
   * - Inactive sessions (reactivates them)
   * - New sessions (creates them)
   * - Race conditions (handles duplicate key violations)
   *
   * @param userId - The user ID who owns the session
   * @param companyName - The company name (not used, derived from domain)
   * @param domain - The company domain
   * @returns The created or existing session
   */
  async getOrCreateSession(userId: string, companyName: string, domain: string): Promise<SessionData> {
    const startTime = Date.now()

    this.logger.breadcrumb('SESSION_MANAGER', 'Getting or creating session', {
      userId,
      companyName,
      domain
    })

    this.logger.info('SESSION_MANAGER', 'Getting or creating session', { userId, companyName, domain })

    try {
      // CRITICAL: Use getOrCreateUserSession to handle all edge cases
      // The companyName parameter is ignored - it's derived from domain
      const sessionData = await this.repository.getOrCreateUserSession(userId, domain)
