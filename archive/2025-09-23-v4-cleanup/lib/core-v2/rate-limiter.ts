...
export class RateLimiter {
  ...
  async executeWithRateLimit<T>(
    ...
    if (window.requests.length >= window.limit) {
      permanentLogger.info('Request queued due to rate limit', {
        category: 'RATE_LIMIT',
        model,
    ...
    if (lastRequest && config.minSpacingMs) {
      ...
      if (timeSinceLastRequest < config.minSpacingMs) {
        ...
        permanentLogger.info('RATE_LIMIT', `Waiting ${waitTime}ms for minimum spacing`, {
          model,
          waitTime
        })
  ...
  private async handleRateLimitError<T>(
    ...
    permanentLogger.info('RATE_LIMIT', `Rate limited, retrying after ${delay}ms`, {
      model,
      attempt: attempts,
      delay,
      error: error.message
  ...
  private queueRequest<T>(
    ...
    return new Promise((resolve, reject) => {
      ...
      permanentLogger.info('RATE_LIMIT', 'Request queued', {
        model,
    ...
    try {
      while (queue.length > 0) {
        ...
        if (window.requests.length >= window.limit) {
          ...
          permanentLogger.info('RATE_LIMIT', `Queue waiting ${waitTime}ms for window to clear`, {
            model,
  ...
  reset() {
    ...
    permanentLogger.info('RATE_LIMIT', 'Rate limiter reset')
  }
