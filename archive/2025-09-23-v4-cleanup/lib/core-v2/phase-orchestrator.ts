...
export class PhaseOrchestrator extends EventEmitter {
  ...
  // packStore removed - using CompanyIntelligenceRepository instead
  ...
  constructor() {
    super()
    permanentLogger.info('PHASE_ORCHESTRATOR', 'Initializing Phase-Based Orchestrator', {
    ...
    // PackStore removed - data is stored in company_intelligence_sessions table
  ...
  async initializeSession(request: CompanyResearchRequest): Promise<ResearchSession> {
    ...
    permanentLogger.info('SESSION_INIT', 'Creating new research session', {
      sessionId,
  ...
  async executePhase(
    ...
    permanentLogger.info('PHASE_EXECUTION', `Starting phase: ${phase}`, {
      sessionId,
      phase,
      autoApprove,
    ...
    try {
      ...
      permanentLogger.info('PHASE_COMPLETE', `Phase ${phase} completed`, {
        sessionId,
      ...
      if (phaseControl?.stopAfter === phase) {
        ...
        permanentLogger.info('SESSION_STOP', `Stopping after ${phase} as configured`, {
          sessionId
        })
      } else if (!autoApprove && phaseControl?.requireApproval) {
        session.status = 'awaiting_review'
        permanentLogger.info('APPROVAL_REQUIRED', `Phase ${phase} requires approval`, {
          sessionId,
      ...
      permanentLogger.info('PHASE_ERROR', `Phase ${phase} failed`, {
        sessionId,
        error: error instanceof Error ? error.message : String(error)
      })
  ...
  private async executeScraping(session: ResearchSession): Promise<PhaseResult> {
    permanentLogger.info('SCRAPING_PHASE', '=== STARTING MULTI-PHASE SCRAPING ===', {
    ...
    try {
      ...
      permanentLogger.info('Pages to scrape', {
        category: 'SCRAPING_PHASE',
      ...
      permanentLogger.info('SCRAPING_PHASE', '📋 Phase 1: Rapid Scrape starting', {
        scraper: 'Cheerio',
      ...
      for (let i = 0; i < pagesToScrape.length; i++) {
        ...
        try {
          ...
          permanentLogger.info('SCRAPING_PHASE', `Phase 1: Scraped ${i + 1}/${pagesToScrape.length}`, { url })
        } catch (error) {
          permanentLogger.info('SCRAPING_ERROR', `Phase 1 failed for ${url}`, {
            error: error instanceof Error ? error.message : String(error)
          })
          phase1Results.set(url, {
            data: null,
      ...
      permanentLogger.info('SCRAPING_PHASE', '✅ Phase 2: Content Validation starting', { totalPages: phase1Results.size})
      ...
      for (const [url, result] of phase1Results) {
        if (!result.data || !result.data.content || result.data.content.length < 100) {
          ...
          permanentLogger.info('SCRAPING_PHASE', 'Page needs enhancement', {
            url,
      ...
      permanentLogger.info('SCRAPING_PHASE', 'Phase 2 complete', {
      ...
      if (pagesNeedingEnhancement.length > 0) {
        ...
        permanentLogger.info('SCRAPING_PHASE', '🚀 Phase 3: Selective Enhancement starting', {
          scraper: 'Playwright',
        ...
        for (const url of pagesNeedingEnhancement) {
          try {
            ...
            permanentLogger.info('SCRAPING_PHASE', 'Phase 3: Enhanced page', { url})
          } catch (error) {
            permanentLogger.info('SCRAPING_ERROR', `Phase 3 failed for ${url}`, {
              error: error instanceof Error ? error.message : String(error)
            })
          }
        }
      ...
      permanentLogger.info('SCRAPING_PHASE', '✨ All phases complete!', {
  ...
  private async executeExtraction(session: ResearchSession): Promise<PhaseResult> {
    permanentLogger.info('EXTRACTION_PHASE', 'Starting extraction (NO LLM)', { sessionId: session.id})
  ...
  private async executeEnrichment(session: ResearchSession): Promise<PhaseResult> {
    permanentLogger.info('ENRICHMENT_PHASE', '🔴 WARNING: Starting enrichment WITH LLM CALLS', { sessionId: session.id})
  ...
  private async executeGeneration(session: ResearchSession): Promise<PhaseResult> {
    permanentLogger.info('GENERATION_PHASE', '🔴 WARNING: Starting generation WITH LLM', { sessionId: session.id})
  ...
  async approveAndContinue(sessionId: string): Promise<PhaseResult | null> {
    ...
    permanentLogger.info('APPROVAL_GRANTED', 'Continuing to next phase', {
      sessionId,
  ...
  abortSession(sessionId: string): void {
    ...
    if (session) {
      ...
      permanentLogger.info('SESSION_ABORTED', 'Session aborted by user', {
        sessionId,
