...
export class ScraperOrchestrator {
  ...
  async executeForSession(
    ...
    try {
      ...
      // Get URLs from session merged_data.sitemap (database-first!)
      // URLs are now stored in merged_data.sitemap.pages as objects with url property
      const sitemapPages = session.merged_data?.sitemap?.pages || []
      const urls = sitemapPages.map((page: any) =>
