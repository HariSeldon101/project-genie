 ...
 * Run with: npx tsx scripts/test-consistency.ts
 */
...
try {
  ...
  console.log(`   - PID Tables: ${(pidHTML.match(/<table/g) || []).length}`)
  console.log(`   - BC Tables: ${(bcHTML.match(/<table/g) || []).length}`)
  console.log(`   - BC Mermaid Charts: ${(bcHTML.match(/class="mermaid"/g) || []).length}`)
  console.log(`   - PID Sections: ${(pidHTML.match(/<section/g) || []).length}`)
  console.log(`   - BC Sections: ${(bcHTML.match(/<section/g) || []).length}`)
} catch (error: any) {
...
console.log('Both document viewer and PDF generation use the same unified formatters.')
console.log('Visual elements (charts, tables, etc.) are preserved across all views.')
console.log('='.repeat(60))