/**
 * SSE Event Factory - Legacy Event System
 *
 * ⚠️⚠️⚠️ DEPRECATED - MARKED FOR DELETION ⚠️⚠️⚠️
 * Deprecation Date: September 21, 2025
 * Archive Target: /archive/lib/company-intelligence/utils/
 *
 * 🚨 DO NOT USE THIS FILE - WILL BE ARCHIVED IMMEDIATELY
 *
 * REPLACEMENT SYSTEM:
 * ==================
 * Primary: /lib/realtime-events/factories/event-factory.ts
 * Types: /lib/realtime-events/core/event-types.ts
 * Client: /lib/realtime-events/client/stream-reader.ts
 * Server: /lib/realtime-events/server/stream-writer.ts
 *
 * MIGRATION GUIDE:
 * ===============
 ...
 * const event = SSEEventFactory.progress(50, 100, 'Processing...')
 ...
 * streamWriter.write(event)  // Automatic SSE formatting
 * ```
 *
 * KEY IMPROVEMENTS:
 * - Unified event system (no SSE vs notification split)
 ...
 *
 * BREAKING CHANGES:
 * - Import path changed completely
 * - StreamWriter handles SSE formatting (don't manually format)
 * - Event types now in separate file
 * - Client uses StreamReader not EventSource directly
 *
 * @deprecated Since Sept 21, 2025 - Use EventFactory from @/lib/realtime-events
 * @marked-for-deletion Moving to /archive/lib/company-intelligence/utils/
 * @migration-priority HIGH - Core infrastructure component
 * @see /new-scraper-architecture-sept-21st.md for complete migration guide
 */
...
export class SSEEventFactory {
  ...
  static progress(
    ...
    permanentLogger.info('SSE_EVENT_FACTORY', 'Progress event created', {
      id: event.id,
  ...
  static data<T>(
    ...
    permanentLogger.info('SSE_EVENT_FACTORY', 'Data event created', {
      id: event.id,
      sequence: event.sequence,
  ...
  static complete<T>(
    ...
    permanentLogger.info('SSE_EVENT_FACTORY', 'Complete event created', { id: event.id,
      sequence: event.sequence,
      correlationId: event.correlationId,
      hasResult: !!result })
  ...
  static status(
    ...
    permanentLogger.info('SSE_EVENT_FACTORY', 'Status event created', { id: event.id,
      sequence: event.sequence,
      status,
      correlationId: event.correlationId })
