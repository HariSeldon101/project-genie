      ...
      try {
        ...
        if (this.options.sessionId) {
          try {
            ...
            // Use proper error structure that matches EventFactory pattern
            writer.dataUpdate({
              type: 'discovery-failed',
              domain: this.domain,
              // Ensure error data has proper nested structure
              data: {
                message: dbError instanceof Error ? dbError.message : 'Database update failed',
                code: (dbError as any).code || 'DB_UPDATE_ERROR',
                stack: dbError instanceof Error ? dbError.stack : undefined,
                pagesFound: allPages.length // Include how many pages were found
              },
              error: dbError instanceof Error ? dbError.message : 'Database update failed', // Keep for backward compatibility
              sessionId: this.options.sessionId,
              pagesFound: allPages.length // Include at top level for backward compatibility
            })
        ...
        writer.dataUpdate({
          ...
          data: {
            ...
            code: (error as any).code || 'DISCOVERY_ERROR',
            stack: error instanceof Error ? error.stack : undefined
          },
          error: error instanceof Error ? error.message : 'Unknown error occurred', // Keep for backward compatibility
          sessionId: this.options.sessionId
        })
  ...
  private async updateDatabase(result: DiscoveryResult) {
    try {
      ...
      if (this.useRepository) {
        ...
        // Update merged data (URLs are now stored in merged_data.sitemap.pages)
